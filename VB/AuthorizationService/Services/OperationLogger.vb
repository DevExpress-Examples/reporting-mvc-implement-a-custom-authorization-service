...
#Region "IWebDocumentViewerAuthorizationService"
        Private Function CanCreateDocument() As Boolean Implements IWebDocumentViewerAuthorizationService.CanCreateDocument
            Return CheckUserAuthorized()
        End Function

        Private Function CanCreateReport() As Boolean Implements IWebDocumentViewerAuthorizationService.CanCreateReport
            Return CheckUserAuthorized()
        End Function

        Private Function CanReadDocument(ByVal documentId As String) As Boolean Implements IWebDocumentViewerAuthorizationService.CanReadDocument
            Return CheckEntityAvailability(Constants.DocumentDictionaryName, documentId)
        End Function

        Private Function CanReadReport(ByVal reportId As String) As Boolean Implements IWebDocumentViewerAuthorizationService.CanReadReport
            Return CheckEntityAvailability(Constants.ReportDictionaryName, reportId)
        End Function

        Private Function CanReleaseDocument(ByVal documentId As String) As Boolean Implements IWebDocumentViewerAuthorizationService.CanReleaseDocument
            Return CheckEntityAvailability(Constants.DocumentDictionaryName, documentId)
        End Function

        Private Function CanReleaseReport(ByVal reportId As String) As Boolean Implements IWebDocumentViewerAuthorizationService.CanReleaseReport
            Return CheckEntityAvailability(Constants.ReportDictionaryName, reportId)
        End Function
...
#End Region  ' IWebDocumentViewerAuthorizationService
        Private Sub SaveUsedEntityId(ByVal dictionaryName As String, ByVal id As String)
            ...
            SyncLock HttpContext.Current.Session.SyncRoot
                If HttpContext.Current.Session(dictionaryName) Is Nothing Then dictionary = New ConcurrentDictionary(Of String, Boolean)() HttpContext.Current.Session(dictionaryName) = dictionary
            End SyncLock

            If dictionary Is Nothing Then dictionary = CType(HttpContext.Current.Session(dictionaryName), ConcurrentDictionary(Of String, Boolean))
        ...
        Private Function CheckEntityAvailability(ByVal dictionaryName As String, ByVal id As String) As Boolean
            ...
            SyncLock HttpContext.Current.Session.SyncRoot
                If HttpContext.Current.Session(dictionaryName) Is Nothing Then Return False
            End SyncLock

            Return CType(HttpContext.Current.Session(dictionaryName), ConcurrentDictionary(Of String, Boolean)).ContainsKey(id)
        End Function

        Private Sub DisposeEntityRequested(ByVal dictionaryName As String, ByVal id As String)
            If String.IsNullOrEmpty(id) Then Return
            SyncLock HttpContext.Current.Session.SyncRoot
                If HttpContext.Current.Session(dictionaryName) Is Nothing Then Return
            End SyncLock

            Call CType(HttpContext.Current.Session(dictionaryName), ConcurrentDictionary(Of String, Boolean)).AddOrUpdate(id, True, Function(_1, _2) True)
        End Sub
    End Class
End Namespace
